...
Repository Intel      workspace.py      git 工作区：干净门禁 / diff / 回滚兜底 / 文件清单
                      adapters.py       唯一认识具体技术栈的地方（detector 注册表，见下方表格）
...
[`adapters.py`](repopilot/adapters.py) 里。探测是一串小 detector 函数的注册表——
加一种技术栈 = 一个函数 + 一条测试，**核心零改动**。认不出的项目也能用 `--test-cmd` 兜底。
...
| Java (Gradle) | `build.gradle[.kts]` | `./gradlew test`（或 `gradle test`）|
| Ruby | `.rspec` / `spec/` / `Gemfile` | `bundle exec rspec` / `rake test` |
| Node | `package.json` | `npm test` |
| NestJS | `package.json` 里有 `@nestjs/core` | `npm test` |
| _（兜底）_ | `Makefile` 里有 `test:` 目标 | `make test` |
| _任意_ | `--test-cmd "<命令>"` | 你指定的命令 |
