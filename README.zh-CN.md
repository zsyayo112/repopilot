...
- [x] **评测线束 v2 —— 为了经得起追问而重建**
      （[`eval/`](eval/)，[方法论](eval/README.md)）。v1 的 Lite 脚本已作废，
      因为它有一个**已确认的泄露**：agent 的测试范围是从官方 `test_patch` 里抠出
      的测试文件路径，等于告诉了它隐藏测试藏在哪。不是恶意，是顺手 ——
      而顺手是必然会发生的，所以边界不再是纪律，是结构：
      - **推理与判分是两个进程**。`PublicInstance` 只有五个字段；
        `test_patch` / `FAIL_TO_PASS` / `PASS_TO_PASS` / gold 只存在于 `grader.py`。
        推理侧模块的代码里只要出现这些名字，CI 里的 AST 检查就会挂。
        测试范围现在只来自 adapter 探测和仓库自己的测试目录。
      - **SWE-bench Verified + 事前声明的抽样总体**，划分成冻结的
        `dev`(15) / `test`(30) / `holdout`(10)，带校验和提交进 git。
        分母就是冻结清单：装不起来的实例留在分母里单列，不许删。
      - **每次运行一份不可变 manifest**：agent commit、prompt 哈希、
        工具 schema 哈希、预算指纹、实例校验和、判分协议版本。
      - **一个预算对象**贯穿所有变体，在循环里强制执行而不是写在提示词里请求；
        每个效果数字旁边并排给成本，且单列缓存命中价。
      - **三个基线**（单轮 / 朴素 ReAct / 完整版）在**同一批实例上成对比较** ——
        净百分比会把"额外结构反而帮了倒忙"那一列藏起来。
      - **补丁审计**（改测试、加 skip、删断言、钉依赖、留调试输出）与
        **单一主因的失败归因**，让"失败了几条"变成"下一步该改哪里"。
      - 环境伪影靠两条**机械规则**压制：解释器按仓库自己声明的 `requires-python` 选，
        依赖按 base commit 当天的时间线装 —— 取代了 v1 那张手工钉版本表。
        **一张能被调的表，就是一个能被调的分子。**
      **首轮真实结果（dev 划分，冻结 15 / gold 可判定 9）**，而最不舒服的
      那部分正是建它的意义：**完整版 resolved 4/9 —— 和一个朴素 ReAct 循环
      解出的是同一批四条，token 却是它的两倍**（54.5 万 vs 27.0 万，
      9 条里 6 条烧穿预算，ReAct 只有 1 条）。单轮直接写补丁 0/9，
      说明这批题目没有一条是靠读 issue 猜就能对的。
      Reviewer 有没有用**在这个规模上测不出来**：一个阴性对照
      （唯一差别是一个从未被调用的工具）在 9 条里翻了 2 条，
      而 Reviewer 的效应量只有 1 条 —— MoE 模型上 `temperature=0` 不等于确定性，
      成对比较消得掉实例难度差异、消不掉运行间方差。
      轨迹给出了原因：**13 条里 6 条从未调用过 `edit_file`**，预算烧光时
      还停在读代码阶段。瓶颈是没有上下文压缩，而本轮加的机制全在它下游。
      dev 的失败轨迹已被读过并据此改过代码，**它从此只是开发样本**，
      这些数字不能对外当成绩。详见 [`eval/README.md`](eval/README.md)。
- [ ] **Phase 5 深水区（一次挖一个）** Docker 沙箱替代黑名单
      （容器边界是内核级的，子串黑名单不是）/ tree-sitter 或 LSP 替代正则符号提取 /
      上下文压缩 / 依赖图检索 / 用官方 Docker 线束替换环境层
      （它同时也是抽样总体能突破 7 个仓库的前提）
...
- **没有语义级相关性判定**：跑偏护栏都是量上的（文件数、片段唯一命中）。
  它在允许范围内改错了东西，抓不到。
- **没有持久化**：跑挂了从头再来。
- **评测的环境层不是官方 Docker 线束**：两条机械规则（时代对应的解释器、
  按提交日期封顶的依赖）把伪影率压下去了很多，但官方的 per-instance 镜像还能
  更彻底 —— 那也是抽样总体突破 7 个仓库的前提。
- **Reviewer 误杀只能报"疑似"**：要坐实一次误杀，得对**被驳回的那一版**单独判分，
  目前没做。
- **v2 线束还没有对外的成绩数字**：它已在一条真实 SWE-bench Verified 实例上
  端到端验证通过，但冻结划分尚未跑完。v1 的 Lite 数字已作废（泄露，见路线图），
  在任何地方引用它，都是在引用一个透过墙洞量出来的读数。
